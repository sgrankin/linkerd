...
private[netty4] trait Netty4H2Writer extends H2Transport.Writer {
  ...
  override def write(stream: Http2FrameStream, msg: Headers, eos: Boolean): Future[Unit] = {
    val headers = Netty4Message.Headers.extract(msg)
    val frame = new DefaultHttp2HeadersFrame(headers, eos)
    if (stream != null) frame.stream(stream)
    write(frame)
  }

  override def write(stream: Http2FrameStream, f: Frame): Future[Unit] = f match {
    case data: Frame.Data => write(stream, data.buf, data.isEnd)
    case tlrs: Frame.Trailers => write(stream, tlrs, eos = true)
  }

  override def write(stream: Http2FrameStream, buf: Buf, eos: Boolean): Future[Unit] = {
  ...
  override def updateWindow(stream: Http2FrameStream, incr: Int): Future[Unit] = {
    val frame = new DefaultHttp2WindowUpdateFrame(incr)
    if (stream != null) frame.stream(stream)
    write(frame)
  }

  override def reset(stream: Http2FrameStream, rst: Reset): Future[Unit] = {
    require(stream != null)
